  ...
  <div id="content">
    <form method="POST">
      <fieldset>                                                  <!-- languages: options as previously defined in $languageOptions -->
        <legend><h2>Translations</h2></legend>
        <div class="control">
          <?php
            $i = 0;
            foreach($languageOptions as $language) {
              ...
              if($language == '_lat') {
                echo "Offer original in " . $languages[$language];
              } else {
                echo "Offer translation in " . $languages[$language];
              }
              echo    "</label>\n"
      ...
      <fieldset>
        <legend><h2>Annotations</h2></legend>                     <!-- Offer marginalia -->
        <div class="control">
          <label for="margins">Show Annotations</label>
          <?php
      ...
      <fieldset>
        <legend><h2>Commentary</h2></legend>                      <!-- Offer commentary: available options are listed in $commentaryOptions (header.php) -->
        <div class="control">
          <?php
          if (is_array($commentaryOptions) || is_object($commentaryOptions)) {
            foreach($commentaryOptions as $commentary) {
              ...
              if($commentary == "editorsComments") {
                echo "offer editors' comments";
              } else if($commentary == "additionalComments") {
                echo "offer further comments";
              }
              echo  "</label>\n"
      ...
      <label for="margins">Submit Changes</label>           <!-- Submit selection -->
    ...
    ?>" download>Download this chapter as a TEI file</a></p>
  </div>
</div>
<?php
